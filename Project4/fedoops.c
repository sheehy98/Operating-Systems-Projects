...
void *slaveAway(void *arg)
{
  ...
  while (1)
      ...
      while (1)
	{
      ...
      for (int i = 0; i < workerPackage->instructionCount; i++)
	{
	  nextStation = stations + (workerPackage->custInstructions[i] - 1);
	  printf("MOVE:  Worker %s #%d is moving Package #%d to Station %s\n",
		 teamName, workerId, workerPackage->packageNum, nextStation->stationName);
	  ...
	  while (1)
	    {
	      ...
	      stationFreeFlag = nextStation->isFree;
	      if (stationFreeFlag && i > 0){
	      ...
	      else
		{
		  printf("WAIT:  Station %s is currently busy. Worker %s #%d waiting with Package #%d\n",
			 nextStation->stationName, teamName, workerId, workerPackage->packageNum);
