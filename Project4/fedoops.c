  ...
  while (1)
	...
	// prints each of the instructions of the package in order
	{
	  ...
	  while (1)
		  ...
		  pthread_cond_wait(&cond, &stationMtx);  // waits for a worker to finish working on a station before checking station availability again
		  pthread_mutex_unlock(&stationMtx);
                }
            }
	  ...
	  pthread_cond_broadcast(&cond);  // broadcasts the condition variable to every worker currently waiting on a station, allowing them to recheck if they can move where they want to go
        }
