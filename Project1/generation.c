...
int newChildProcess(int descendantCount)
{
    ...
    else if (rc == 0) //child process
    {
        ...
        // if (descendantCount != 0) //if new lifespan/descendant count is non-zero, spawn new child, print to screen
        // {
        ...
        newChildProcess(newDescendantCount); //recusively call itself with the new decreased generation count
        exit(newDescendantCount);            //exit with newDescendantCount to pass to parent
        // }
    }
    ...
    else //parent process
    {
        ...
        {
            descendantCount = WEXITSTATUS(status); //update descendant count value
...
int main(int argc, char *argv[])
{
    ...
    newChildProcess(descendantCount);
